    ...
    <section id="discover" class="w-full max-w-full mx-auto px-6 md:px-12 lg:px-16 py-32 md:py-48">
        ...
        <div class="flex flex-col space-y-40 md:space-y-60 lg:space-y-80">
            @foreach($spots as $spot)
                ...
                <div class="fade-in-item opacity-0 translate-y-12 transition-all duration-[1500ms] ease-out flex flex-col md:flex-row {{ $loop->index % 2 != 0 ? 'md:flex-row-reverse' : '' }} items-center justify-between gap-12 lg:gap-20 w-full">

                    <a href="{{ route('spots.show', $spot->id) }}" class="block w-full md:w-[58%] lg:w-[60%] relative aspect-[16/9] overflow-hidden group shadow-sm">
                        ...
                        <div class="absolute inset-0 bg-[#2C2C2C]/5 group-hover:bg-transparent transition-colors duration-1000"></div>
                    </a>

                    <div class="w-full md:w-[38%] lg:w-[35%] flex flex-col justify-center {{ $loop->index % 2 != 0 ? 'md:items-end md:text-right md:pl-12' : 'md:items-start md:text-left md:pr-12' }} text-center md:text-left">
                        
                        <span class="text-sm md:text-base font-sans tracking-[0.3em] text-[#2C2C2C]/60 uppercase mb-6">Shizuoka, Japan</span>
                        
                        <h3 class="text-4xl lg:text-6xl font-serif font-light text-[#2C2C2C] tracking-wide mb-8">{{ $spot->name }}</h3>
                        
                        <p class="text-lg md:text-xl lg:text-2xl font-sans tracking-wider leading-relaxed text-[#2C2C2C]/80 mb-14 max-w-md md:max-w-none mx-auto md:mx-0">
                            {{ $spot->description }}
                        </p>
                        
                        <a href="{{ route('spots.show', $spot->id) }}" class="group flex items-center gap-6 pb-2 mx-auto md:mx-0">
                            <span class="text-sm md:text-base font-sans tracking-[0.2em] uppercase group-hover:opacity-50 transition-opacity">Explore Detail</span>
                    ...
                    if (!entry.isIntersecting) {
                        ...
                        words.forEach(item => {
                            ...
                            if (isEnglish) {
                                ...
                                textEl.className = 'relative z-0 font-jp font-normal text-6xl md:text-7xl lg:text-[8rem] tracking-[0.2em] pt-2 text-[#2C2C2C]';
                            } else {
