...
<head>
    ...
    <link
        href="https://fonts.googleapis.com/css2?family=Cormorant+Garamond:wght@300;400;600&family=Montserrat:wght@200;300;400&family=Shippori+Mincho:wght@400;500&display=swap"
        rel="stylesheet">
    <style>
        body {
...
<body class="min-h-screen flex flex-col selection:bg-[#2C2C2C] selection:text-[#FAF8F5]">
    ...
    <!-- 修正1: ヘッダーの文字色を白(text-white)からダークグレー(text-[#2C2C2C])に変更 -->
    <header class="w-full px-8 md:px-16 py-12 flex justify-between items-center absolute top-0 z-50">
        <a href="{{ url('/') }}"
            class="text-xs font-sans tracking-[0.4em] uppercase text-[#2C2C2C] hover:opacity-50 transition-opacity">Élysée</a>
        <a href="{{ url('/') }}"
            class="text-[10px] font-sans tracking-[0.3em] uppercase text-[#2C2C2C] hover:opacity-50 transition-opacity">Back
            to Top</a>
    </header>

    <!-- 修正2: 画面いっぱいの画像を廃止し、美しく計算された「余白」の中央に写真を配置 -->
    <div class="w-full max-w-7xl mx-auto px-8 md:px-16 lg:px-24 pt-36 pb-12 relative">
        <!-- 写真をシネマティックな横長比率にし、角を少し丸めて上品な額縁のようにする -->
        <div class="w-full aspect-[16/9] md:aspect-[21/9] relative overflow-hidden rounded-xl shadow-sm group">
            <div class="absolute inset-0 bg-cover bg-center transition-transform duration-[3000ms] group-hover:scale-105"
                style="background-image: url('{{ asset($spot->image_path) }}');"></div>
            <!-- 写真を少しだけ引き締めるための極薄いフィルター -->
            <div class="absolute inset-0 bg-black/10"></div>
        </div>
    </div>

    <main class="max-w-4xl mx-auto px-8 py-32 md:py-48 text-center">
        <span class="text-sm md:text-base font-sans tracking-[0.3em] text-[#2C2C2C]/60 uppercase mb-8 block">Shizuoka,
            Japan</span>
    ...
    <section class="w-full bg-white px-8 py-32 border-t border-[#2C2C2C]/10">
        <div class="max-w-3xl mx-auto">
            <div class="text-center mb-24">
                <h2 class="text-3xl md:text-4xl font-serif font-light text-[#2C2C2C] tracking-wide mb-6">Personal
                    Consultation</h2>
            ...
            @if(session('success'))
                <div
                    class="mb-12 p-6 border border-[#2C2C2C]/20 text-center font-sans tracking-wider text-[#2C2C2C] bg-[#FAF8F5]">
                    {{ session('success') }}
                </div>
            @endif
            ...
            <form action="{{ route('spots.reserve', $spot->id) }}" method="POST" class="flex flex-col gap-12 font-sans">
                ...
                <div>
                    <label class="block text-xs tracking-[0.2em] uppercase text-[#2C2C2C]/60 mb-4">Preferred
                        Date</label>
                ...
                <div>
                    <label class="block text-xs tracking-[0.2em] uppercase text-[#2C2C2C]/60 mb-4">Email Address</label>
                    <input type="email" name="email" value="{{ old('email') }}" placeholder="your@email.com"
                        class="w-full bg-transparent border-b border-[#2C2C2C]/30 pb-4 text-lg focus:border-[#2C2C2C] outline-none transition-colors">
                    @error('email') <p class="text-red-500 text-xs tracking-widest mt-2">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label class="block text-xs tracking-[0.2em] uppercase text-[#2C2C2C]/60 mb-4">Number of
                        Guests</label>
                ...
                <div>
                    <label class="block text-xs tracking-[0.2em] uppercase text-[#2C2C2C]/60 mb-4">Your
                        Preferences</label>
                ...
                <div class="mt-8 text-center">
                    <button type="submit"
                        class="group flex items-center justify-center gap-6 mx-auto hover:opacity-50 transition-opacity">
